...
def followup_sequence(inputs):
    customer = inputs["customer"] or "there"; project_type = inputs["project_type"].lower(); company = inputs["company"] or "our team"
...
with st.sidebar:
    st.title("FollowUpPilot AI"); st.caption("Version 2.3")
...
with copy_tabs[0]: st.text_area("Text", sms, height=145)
with copy_tabs[1]:
    st.text_input("Email Subject", subject)
    st.text_area("Email Body", email, height=260)
with copy_tabs[2]: st.text_area("Voicemail", voicemail, height=145)
with copy_tabs[3]: st.text_area("CRM Note", crm, height=260)
with copy_tabs[4]: st.text_area("Manager Coaching Note", coaching, height=145)
...
html_card("Portfolio Skills Shown", "<ul><li>Workflow mapping</li><li>Rules-based next-best-action logic</li><li>Sales communication generation</li><li>CRM documentation support</li><li>Downloadable business reports</li></ul>", "workflow-card")

with st.expander("How to use FollowUpPilot AI"):
    st.markdown("1. Load a sample scenario or enter your own customer/project context.\n2. Generate the follow-up plan.\n3. Use the Copy Center for text, email, voicemail, CRM note, and coaching note.\n4. Review the follow-up timeline.\n5. Download the follow-up plan for CRM documentation or team coaching.")
